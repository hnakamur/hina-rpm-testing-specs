...
%define CONFIG_FLAGS -DSSL_ALLOW_ADH --prefix=%{_prefix} --openssldir=%{openssldir}
...
echo %{_libdir} > $RPM_BUILD_ROOT/etc/ld.so.conf.d/openssl-%{version}.conf
...
%config %{_sysconfdir}/ld.so.conf.d/openssl-%{version}.conf
